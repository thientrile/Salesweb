...
class admin
{
    ...
    function userRegister($username, $email, $pass, $phonenumber = null, $address = null, $type)
    {
        if ($this->check && ($this->role == 1 || $this->role == 3 || $this->role == 7)) {
            ...
            if ($this->checkEmailExit($email) == false) {
                ...
                // if (
                //     mkdir("assets/user/" . md5($db->getonce($select)[0]), 0777) &&
                //     mkdir("assets/user/" . md5($db->getonce($select)[0]) . "/avatar", 0777)
                // ) {
                //     $result = $db->send($query);
                // }
                mkdir("assets/user/" . md5($db->getonce($select)[0]), 0755);
                mkdir("assets/user/" . md5($db->getonce($select)[0]) . "/avatar", 0755);
    ...
    function insertProduct($post, $files)
    {
        ...
        mkdir("assets/products/" . md5($id), 0777, true);
        mkdir("assets/products/" . md5($id) . "/img", 0777, true);
        mkdir("assets/products/" . md5($id) . "/gallery", 0777, true);
        mkdir("assets/products/" . md5($id) . "/src", 0777, true);
