        ...
        while ($row = $result->fetch()) {
            ...
            if ($resutl_items->rowCount() > 1) {
                ...
                while ($item = $resutl_items->fetch()) {
                    ...
                    array_push($option, array("id" => $item['id'], "price" => $item['price'], "sources" => $item['sources'], "checked" =>  $check, "cart" => $checkCart));
                }
            }
    ...
    function viewProductDetails($productsId)
    {
        ...
        if ($resutl_items->rowCount() > 1) {
            ...
            while ($item = $resutl_items->fetch()) {
                ...
                array_push($option, array("id" => $item['id'], "price" => $item['price'], "sources" => $item['sources'], "check" => $check,"cart" => $checkCart));
            }
        }
